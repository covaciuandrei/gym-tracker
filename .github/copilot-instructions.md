...
| `legal_consent_checkbox.dart`      | Mandatory GDPR-consent checkbox used above the Register submit button. Built on top of `LabeledCheckbox`, with a localized rich-text sentence (ICU `legalConsentMessage` template with `{terms}`/`{privacy}` placeholders — translators control word order) injected into the `label` slot. Takes two parent-owned `ValueNotifier<bool>`s (`accepted`, `showError`) plus two pre-resolved URL strings (`termsUrl`, `privacyUrl`). **URL resolution is intentionally not performed here** — the parent (`RegisterPage`) resolves them via `AppVersionStatus.termsUrlFor(languageCode)` / `.privacyUrlFor(languageCode)` (3-tier fallback: requested locale → English entry → hardcoded constant in `lib/core/constants/legal_urls.dart`). Taps delegate to `launchLegalUrl()` (`lib/core/utils/legal_url_launcher.dart`), which enforces **https-only** (rejects `http`, custom schemes, `javascript:`), requires a non-empty host, and dismisses the keyboard before launching — defense-in-depth against a misconfigured `appConfig` document. When `showError` is true, an inline `legalConsentRequired` error appears via `LabeledCheckbox.errorText`; ticking the checkbox auto-clears `showError`. Accessibility: a plain sentence (`_plainSentence`) is set on the `Semantics.label` alongside `ExcludeSemantics`-wrapped rich text so screen readers speak a clean string. |
