...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockUserService.recordLogin(userId: any(named: 'userId'))).thenAnswer((_) async {});
    when(
  ...
  group('watchAuthState', () {
    test('emits AuthAuthenticatedState when user is logged in', () async {
      when(() => mockService.currentUser$).thenAnswer((_) => Stream.value(_fakeUser));

      final future = expectLater(sut.stream, emitsInOrder([isA<AuthAuthenticatedState>()]));
      sut.watchAuthState();
      await future;
    });

    test('emits AuthUnauthenticatedState when stream emits null', () async {
      when(() => mockService.currentUser$).thenAnswer((_) => Stream.value(null));

      final future = expectLater(sut.stream, emitsInOrder([const AuthUnauthenticatedState()]));
      sut.watchAuthState();
      await future;
    });
  });
  ...
  group('signIn', () {
    test('emits pending then success with user', () async {
      when(() => mockService.signIn(email: _email, password: _password)).thenAnswer((_) async => _fakeUser);

      final future = expectLater(
        sut.stream,
        emitsInOrder([const PendingState(), isA<AuthSignInSuccessState>().having((s) => s.user, 'user', _fakeUser)]),
      );
    ...
    test('emits pending then invalidCredentials', () async {
      when(() => mockService.signIn(email: _email, password: _password)).thenThrow(const InvalidCredentialsException());

      final future = expectLater(sut.stream, emitsInOrder([const PendingState(), const AuthInvalidCredentialsState()]));
      await sut.signIn(email: _email, password: _password);
      await future;
    });

    test('emits pending then emailNotVerified', () async {
      when(() => mockService.signIn(email: _email, password: _password)).thenThrow(const EmailNotVerifiedException());

      final future = expectLater(sut.stream, emitsInOrder([const PendingState(), const AuthEmailNotVerifiedState()]));
      await sut.signIn(email: _email, password: _password);
      await future;
    });

    test('emits pending then somethingWentWrong on unknown error', () async {
      when(() => mockService.signIn(email: _email, password: _password)).thenThrow(Exception('unknown'));
  ...
  group('signUp', () {
    test('emits pending then signUpSuccess', () async {
      when(() => mockService.signUp(email: _email, password: _password)).thenAnswer((_) async => _fakeUser);
      final future = expectLater(sut.stream, emitsInOrder([const PendingState(), const AuthSignUpSuccessState()]));
      await sut.signUp(
        email: _email,
        password: _password,
        displayName: _displayName,
        consent: const <String, Object?>{'termsAccepted': true},
      );
      await future;
    });

    test('emits emailAlreadyInUse', () async {
      when(() => mockService.signUp(email: _email, password: _password)).thenThrow(const EmailAlreadyInUseException());

      final future = expectLater(sut.stream, emitsInOrder([const PendingState(), const AuthEmailAlreadyInUseState()]));
      await sut.signUp(
        email: _email,
        password: _password,
        displayName: _displayName,
    ...
    test('emits weakPassword', () async {
      when(() => mockService.signUp(email: _email, password: _password)).thenThrow(const WeakPasswordException());

      final future = expectLater(sut.stream, emitsInOrder([const PendingState(), const AuthWeakPasswordState()]));
      await sut.signUp(
        email: _email,
        password: _password,
        displayName: _displayName,
  ...
  group('signOut', () {
    test('emits pending then signOutSuccess', () async {
      ...
      final future = expectLater(sut.stream, emitsInOrder([const PendingState(), const AuthSignOutSuccessState()]));
      await sut.signOut();
      await future;
    });
  });
  ...
  group('resetPassword', () {
    test('emits pending then passwordResetSent', () async {
      ...
      final future = expectLater(sut.stream, emitsInOrder([const PendingState(), const AuthPasswordResetSentState()]));
      await sut.resetPassword(email: _email);
      await future;
    });

    test('emits somethingWentWrong on failure', () async {
      when(() => mockService.resetPassword(_email)).thenThrow(Exception('network'));
  ...
  group('changePassword', () {
    ...
    test('emits pending then passwordChanged', () async {
      when(() => mockService.changePassword(currentPassword: _password, newPassword: newPass)).thenAnswer((_) async {});

      final future = expectLater(sut.stream, emitsInOrder([const PendingState(), const AuthPasswordChangedState()]));
      await sut.changePassword(currentPassword: _password, newPassword: newPass);
      await future;
    });

    test('emits invalidCredentials when current password is wrong', () async {
      when(
        () => mockService.changePassword(currentPassword: 'wrong', newPassword: newPass),
  ...
  group('deleteAccount', () {
    test('emits pending then accountDeleted on success', () async {
      when(() => mockService.reauthenticate(currentPassword: _password)).thenAnswer((_) async {});
      when(() => mockService.currentUserId).thenReturn('uid_001');
      when(() => mockCleanupService.deleteAllUserData(userId: 'uid_001')).thenAnswer((_) async {});
      when(() => mockService.deleteAccount()).thenAnswer((_) async {});

      final future = expectLater(sut.stream, emitsInOrder([const PendingState(), const AuthAccountDeletedState()]));
      await sut.deleteAccount(currentPassword: _password);
      await future;

      verify(() => mockCleanupService.deleteAllUserData(userId: 'uid_001')).called(1);
    ...
    test('emits invalidCredentials when password is wrong', () async {
      when(() => mockService.reauthenticate(currentPassword: 'wrong')).thenThrow(const InvalidCredentialsException());

      final future = expectLater(sut.stream, emitsInOrder([const PendingState(), const AuthInvalidCredentialsState()]));
      await sut.deleteAccount(currentPassword: 'wrong');
      await future;

      verifyNever(() => mockCleanupService.deleteAllUserData(userId: any(named: 'userId')));
    ...
    test('emits somethingWentWrong when cleanup fails', () async {
      when(() => mockService.reauthenticate(currentPassword: _password)).thenAnswer((_) async {});
      when(() => mockService.currentUserId).thenReturn('uid_001');
      when(() => mockCleanupService.deleteAllUserData(userId: 'uid_001')).thenThrow(Exception('firestore error'));
