...
class HealthService {
  ...
  static String yearMonthKey(int year, int month) => '$year-${month.toString().padLeft(2, '0')}';

  /// Streams the full global supplement product catalog, ordered by name.
  Stream<List<SupplementProduct>> watchAllProducts() => _source.watchAllProducts();

  /// Streams only products created by [userId].
  Stream<List<SupplementProduct>> watchMyProducts(String userId) => _source.watchMyProducts(userId);

  /// Returns a single [SupplementProduct] by [productId], or null.
  Future<SupplementProduct?> getProduct(String productId) => _source.getProduct(productId);

  /// Creates a new supplement product. Returns the generated document id.
  Future<String> createProduct(SupplementProduct model) => _source.createProduct(model);
  ...
  Future<void> deleteProduct(String productId) => _source.deleteProduct(productId);
  ...
  Future<String> logSupplement({required String userId, required SupplementLog model}) {
