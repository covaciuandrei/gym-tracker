...
class SupplementMapper {
  ...
  SupplementProductDto mapProductModel(SupplementProduct model) => SupplementProductDto(
    id: model.id,
    name: model.name,
    brand: model.brand,
  ...
  SupplementLogDto mapLogModel(SupplementLog model) => SupplementLogDto(
    ...
    timestamp: model.timestamp == null ? null : Timestamp.fromDate(model.timestamp!),
  );

  ProductIngredient _mapIngredientDto(ProductIngredientDto dto) =>
      ProductIngredient(stdId: dto.stdId, name: dto.name, amount: dto.amount, unit: dto.unit);

  ProductIngredientDto _mapIngredientModel(ProductIngredient model) =>
      ProductIngredientDto(stdId: model.stdId, name: model.name, amount: model.amount, unit: model.unit);
